...
describe('reconcileHudForPromptSubmit', () => {
  ...
  it('recreates a missing HUD in tmux', async () => {
    ...
    const result = await reconcileHudForPromptSubmit('/repo', {
      ...
      resolveRcsCliEntryPath: () => '/repo/dist/cli/rcs.js',
    });

    assert.equal(result.status, 'recreated');
    assert.equal(result.paneId, '%9');
    assert.equal(created.length, 1);
    assert.match(created[0]?.cmd || '', /\/repo\/dist\/cli\/rcs\.js' hud --watch/);
  ...
  it('prefers an explicit session override when recreating HUD', async () => {
    ...
    const result = await reconcileHudForPromptSubmit('/repo', {
      env: { TMUX: '1', TMUX_PANE: '%1', RCS_SESSION_ID: 'sess-stale' },
    ...
    assert.equal(result.status, 'recreated');
    assert.equal(created.length, 1);
    assert.match(created[0]?.cmd || '', /^RCS_SESSION_ID='sess-canonical' node '.*rcs\.js' hud --watch/);
