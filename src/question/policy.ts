...
function hasWorkerContext(env: NodeJS.ProcessEnv): boolean {
  return safeString(env.RCS_TEAM_WORKER).trim() !== '';
}
...
export async function evaluateQuestionPolicy(
  ...
  if (hasWorkerContext(env)) {
    return {
      ...
      sessionId,
      code: 'worker_blocked',
      message: 'rcs question is unavailable for RCS team workers; only non-team leader sessions may ask user questions.',
  ...
  if (activeTeams.length > 0) {
    ...
    return {
      ...
      sessionId,
      code: 'team_blocked',
      message: `rcs question is unavailable while this session owns active team mode: ${summary}.`,
      fallbackAllowed: false,
      activeModes,
      activeSkills,
      activeTeams,
  ...
  if (blocked.length > 0) {
    return {
      ...
      message: `rcs question is unavailable while auto-executing workflows are active: ${blocked.join(', ')}.`,
      fallbackAllowed: false,
      activeModes,
      activeSkills,
      activeTeams,
