...
const ROLE_KEYWORDS: ReadonlyArray<{ role: string; keywords: readonly string[] }> = [
  ...
  { role: 'designer', keywords: ['ui', 'hud', 'menu', 'plugin', 'layout', 'design', 'responsive', 'screengui', 'fusion', 'roact', 'styling', 'ux', '디자인', '레이아웃', '컴포넌트'] },
  ...
  { role: 'code-reviewer', keywords: ['review', 'audit', 'quality', 'lint', 'anti-pattern', 'code review', '검토', '리뷰'] },
...
export function routeTaskToRole(
  ...
  if (intent === 'review') {
    return {
      role: SECURITY_DOMAIN.test(text) ? 'security-reviewer' : 'code-reviewer',
      confidence: 'high',
