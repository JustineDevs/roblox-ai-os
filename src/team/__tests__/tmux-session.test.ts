...
describe('sendToWorker validation', () => {
  it('rejects text over 200 chars', async () => {
    await assert.rejects(
      sendToWorker('rcs-team-x', 1, 'a'.repeat(200)),
      /< 200/i
    );
  });
  ...
  it('rejects injection marker', async () => {
    await assert.rejects(
      sendToWorker('rcs-team-x', 1, `hello [RCS_TMUX_INJECT]`),
      /marker/i
    );
  });
...
`,
  ...
  it('ignores stale queued-next-tool-call banner text that only survives in scrollback history', async () => {
    await withMockTmuxFixture(
      'rcs-tmux-codex-stale-queued-scrollback-',
      (logPath) => `#!/bin/sh
set -eu
state_dir="$(dirname "${logPath}")"
...
`,
  ...
  it('keeps nudging Codex when the visible pane still shows a live queued-next-tool-call banner', async () => {
    await withMockTmuxFixture(
      'rcs-tmux-codex-visible-queued-submit-',
      (logPath) => `#!/bin/sh
set -eu
state_dir="$(dirname "${logPath}")"
text_sent_file="$state_dir/text-sent"
...
`,
  ...
  it('fails closed when the visible queued-next-tool-call banner never clears after the final submit round', async () => {
    await withMockTmuxFixture(
      'rcs-tmux-codex-stuck-queued-submit-',
      (logPath) => `#!/bin/sh
set -eu
state_dir="$(dirname "${logPath}")"
text_sent_file="$state_dir/text-sent"
...
`,
      async ({ logPath }) => {
        await assert.rejects(
          () => sendToWorker('rcs-team-x', 1, 'check inbox'),
...
`,
      async ({ logPath }) => {
        assert.deepEqual(
          await checkWorkerStartupInjectSafety('rcs-team-x', 1),
...
describe('buildWorkerStartupCommand', () => {
  it('auto-selects gemini worker CLI from gemini model', () => {
    ...
    try {
      ...
      if (typeof prevCli === 'string') process.env.RCS_TEAM_WORKER_CLI = prevCli;
      else delete process.env.RCS_TEAM_WORKER_CLI;
      if (typeof prevBypass === 'string') process.env.RCS_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
      else delete process.env.RCS_BYPASS_DEFAULT_SYSTEM_PROMPT;
    }
  });
  ...
  it('injects model_instructions_file override by default', () => {
    ...
    try {
      ...
      if (typeof prevInstr === 'string') process.env.RCS_MODEL_INSTRUCTIONS_FILE = prevInstr;
      else delete process.env.RCS_MODEL_INSTRUCTIONS_FILE;
    }
  });
  ...
  it('uses per-worker RCS_MODEL_INSTRUCTIONS_FILE from extraEnv when building process launch spec', () => {
    ...
    try {
      const spec = buildWorkerProcessLaunchSpec(
        ...
        ['-c', 'model_reasoning_effort="low"'],
        '/tmp/project',
        { RCS_MODEL_INSTRUCTIONS_FILE: '/tmp/project/.rcs/state/team/alpha/workers/worker-1/AGENTS.md' },
        'codex',
      );
  ...
  it('translates model_instructions_file path for MSYS2/Git Bash environments', () => {
    ...
    try {
      ...
      if (typeof prevInstructions === 'string') process.env.RCS_MODEL_INSTRUCTIONS_FILE = prevInstructions;
      else delete process.env.RCS_MODEL_INSTRUCTIONS_FILE;
  ...
  it('uses a native PowerShell startup command on native Windows instead of /bin/sh -lc', async () => {
    ...
    try {
      ...
      if (typeof prevLeaderNodePath === 'string') process.env.RCS_LEADER_NODE_PATH = prevLeaderNodePath;
      else delete process.env.RCS_LEADER_NODE_PATH;
      if (typeof prevMsystem === 'string') process.env.MSYSTEM = prevMsystem;
      else delete process.env.MSYSTEM;
...
describe('team worker CLI helpers', () => {
  ...
  it('resolveTeamWorkerCliPlan supports mixed per-worker CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(
      4,
      [],
      { RCS_TEAM_WORKER_CLI_MAP: 'codex,codex,gemini,claude' },
    );
  ...
  it('resolveTeamWorkerCliPlan accepts single-value map and expands to all workers', () => {
    const plan = resolveTeamWorkerCliPlan(
      3,
      [],
      { RCS_TEAM_WORKER_CLI_MAP: 'claude' },
    );
  ...
  it('resolveTeamWorkerCliPlan supports auto entries in CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(
      2,
      ['--model', 'claude-3-7-sonnet'],
      { RCS_TEAM_WORKER_CLI_MAP: 'auto,codex' },
    );
  ...
  it('resolveTeamWorkerCliPlan rejects map lengths that do not match workerCount', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(4, [], { RCS_TEAM_WORKER_CLI_MAP: 'codex,claude' }),
      /expected 1 or 4/i,
    );
  });
  ...
  it('resolveTeamWorkerCliPlan rejects empty entries in CLI map', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(2, [], { RCS_TEAM_WORKER_CLI_MAP: 'codex,' }),
  ...
  it('resolveTeamWorkerCliPlan reports invalid entry errors with RCS_TEAM_WORKER_CLI_MAP', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(1, [], { RCS_TEAM_WORKER_CLI_MAP: 'claudee' }),
      /RCS_TEAM_WORKER_CLI_MAP/i,
    );
  });

  it('resolveWorkerCliForSend prioritizes explicit worker CLI over map/global', () => {
    assert.equal(
      resolveWorkerCliForSend(2, 'claude', [], { RCS_TEAM_WORKER_CLI_MAP: 'codex,codex' }),
      'claude',
    );
  });
  ...
  it('resolveWorkerCliForSend resolves per-worker map entry by index', () => {
    assert.equal(
      resolveWorkerCliForSend(2, undefined, [], { RCS_TEAM_WORKER_CLI_MAP: 'codex,claude' }),
      'claude',
    );
  });
...
describe('team worker launch mode helpers', () => {
  ...
  it('resolveTeamWorkerLaunchMode rejects unsupported values', () => {
    assert.throws(
      () => resolveTeamWorkerLaunchMode({ RCS_TEAM_WORKER_LAUNCH_MODE: 'tmux' }),
  ...
  it('buildWorkerProcessLaunchSpec returns command/args/env for prompt process spawn', () => {
    ...
    try {
      const spec = buildWorkerProcessLaunchSpec(
        ...
        ['--model', 'gpt-5.3-codex'],
        '/tmp/workspace',
        { RCS_TEAM_STATE_ROOT: '/tmp/workspace/.rcs/state' },
        'codex',
      );
  ...
  it('buildWorkerProcessLaunchSpec includes leader node and CLI path env vars', () => {
    ...
    try {
      ...
      assert.ok(
        typeof spec.env.RCS_LEADER_NODE_PATH === 'string' && spec.env.RCS_LEADER_NODE_PATH.length > 0,
        'RCS_LEADER_NODE_PATH must be set',
      );
      assert.ok(
        typeof spec.env.RCS_LEADER_CLI_PATH === 'string' && spec.env.RCS_LEADER_CLI_PATH.length > 0,
        'RCS_LEADER_CLI_PATH must be set',
      );
...
describe('tmux-dependent functions when tmux is unavailable', () => {
  ...
  it('waitForWorkerReady uses visible capture-pane argv without tail flags', async () => {
    await withMockTmuxFixture(
      'rcs-tmux-worker-ready-visible-capture-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
  ...
  it('waitForWorkerReady falls back to recent scrollback when a live Codex viewport pushes the prompt below the visible slice', async () => {
    await withMockTmuxFixture(
      'rcs-tmux-worker-ready-scrollback-fallback-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
  ...
  it('waitForWorkerReady does not consult scrollback when the visible slice is only status text', async () => {
    await withMockTmuxFixture(
      'rcs-tmux-worker-ready-no-scrollback-status-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
  ...
  it('waitForWorkerReady leaves the Claude bypass prompt untouched when auto-accept is disabled', async () => {
    ...
    try {
      await withMockTmuxFixture(
        'rcs-tmux-claude-bypass-blocked-',
        (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
      ...
      if (typeof previousAutoAccept === 'string') process.env.RCS_TEAM_AUTO_ACCEPT_BYPASS = previousAutoAccept;
      else delete process.env.RCS_TEAM_AUTO_ACCEPT_BYPASS;
    }
  });
...
describe('waitForWorkerReadyAsync parity', () => {
  it('uses visible capture-pane argv without tail flags', async () => {
    await withMockTmuxFixture(
      'rcs-tmux-worker-ready-async-visible-capture-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
  ...
  it('falls back to recent scrollback only when visible slice shows a live Codex viewport', async () => {
    await withMockTmuxFixture(
      'rcs-tmux-worker-ready-async-scrollback-fallback-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
    ...
    await withMockTmuxFixture(
      'rcs-tmux-worker-ready-async-no-scrollback-status-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
  ...
  it('auto-accepts trust prompts and then observes readiness', async () => {
    ...
    try {
      await withMockTmuxFixture(
        'rcs-tmux-worker-ready-async-trust-',
        (logPath) => `#!/bin/sh
set -eu
state_dir="$(dirname "${logPath}")"
accepted_file="$state_dir/accepted"
...
`,
      ...
      if (typeof previousAutoTrust === 'string') process.env.RCS_TEAM_AUTO_TRUST = previousAutoTrust;
      else delete process.env.RCS_TEAM_AUTO_TRUST;
    }
  });
  ...
  it('auto-accepts the Claude bypass prompt and then observes readiness', async () => {
    ...
    try {
      await withMockTmuxFixture(
        'rcs-tmux-worker-ready-async-claude-bypass-',
      (logPath) => `#!/bin/sh
set -eu
state_dir="$(dirname "${logPath}")"
accepted_file="$state_dir/accepted"
...
`,
  ...
  it('returns false on timeout or tmux command failure', async () => {
    await withMockTmuxFixture(
      'rcs-tmux-worker-ready-async-capture-failure-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
  ...
  it('restores standalone HUD panes with an absolute RCS entry path after cwd drift', async () => {
    ...
    try {
      ...
      await withMockTmuxFixture(
        'rcs-tmux-relative-standalone-hud-',
        (logPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${logPath}"
case "\${1:-}" in
...
`,
      ...
      if (typeof previousEntryPath === 'string') process.env[RCS_ENTRY_PATH_ENV] = previousEntryPath;
      else delete process.env[RCS_ENTRY_PATH_ENV];
      if (typeof previousStartupCwd === 'string') process.env[RCS_STARTUP_CWD_ENV] = previousStartupCwd;
      else delete process.env[RCS_STARTUP_CWD_ENV];
      await rm(cwd, { recursive: true, force: true });
  ...
  it('restores standalone HUD panes with the packaged CLI entry when argv1 is not the RCS CLI', async () => {
    ...
    try {
      await withMockTmuxFixture(
        'rcs-tmux-noncli-standalone-hud-',
        (logPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${logPath}"
case "\${1:-}" in
...
describe('dismissTrustPromptIfPresent capture shape', () => {
  it('uses visible capture-pane argv without tail flags', async () => {
    ...
    try {
      await withMockTmuxFixture(
        'rcs-tmux-dismiss-trust-visible-capture-',
        (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
describe('dismissTrustPromptIfPresent', () => {
  ...
  it('returns false when RCS_TEAM_AUTO_TRUST is disabled', () => {
    ...
    try {
      ...
      if (typeof prev === 'string') process.env.RCS_TEAM_AUTO_TRUST = prev;
      else delete process.env.RCS_TEAM_AUTO_TRUST;
    }
  });
...
`,
      async ({ logPath }) => {
        ...
        assert.match(
          tmuxLog,
          /set-option -t rcs-team-x mode-style bg=yellow,fg=black,underscore,nounderscore,nodouble-underscore,nocurly-underscore,nodotted-underscore,nodashed-underscore/,
        );
...
`,
      async ({ logPath }) => {
        ...
        assert.match(
          tmuxLog,
          /set-option -t rcs-team-x copy-mode-selection-style fg=white,bg=blue,curly-underscore,nounderscore,nodouble-underscore,nocurly-underscore,nodotted-underscore,nodashed-underscore/,
        );
