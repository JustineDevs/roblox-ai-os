...
function repo(): string {
  const cwd = mkdtempSync(join(tmpdir(), 'rcs-dag-'));
  mkdirSync(join(cwd, '.rcs', 'plans'), { recursive: true });
  mkdirSync(join(cwd, 'src', 'team'), { recursive: true });
  writeFileSync(join(cwd, 'src', 'team', 'runtime.ts'), '');
  writeFileSync(join(cwd, '.rcs', 'plans', 'prd-demo.md'), '# Demo\n');
  writeFileSync(join(cwd, '.rcs', 'plans', 'test-spec-demo.md'), '# Tests\n');
  return cwd;
}
...
describe('buildRepoAwareTeamExecutionPlan', () => {
  ...
  it('imports DAG sidecar, reduces implicit worker count, and preserves symbolic dependencies for runtime remap', () => {
    const cwd = repo();
    writeFileSync(join(cwd, '.rcs', 'plans', 'team-dag-demo.json'), JSON.stringify({
      schema_version: 1,
      nodes: [
  ...
  it('requires a matching approved test spec before importing an opted-in DAG sidecar', () => {
    const cwd = repo();
    writeFileSync(join(cwd, '.rcs', 'plans', 'test-spec-demo.md'), '');
    writeFileSync(join(cwd, '.rcs', 'plans', 'test-spec-other.md'), '# Other tests\n');
    writeFileSync(join(cwd, '.rcs', 'plans', 'team-dag-demo.json'), JSON.stringify({
      schema_version: 1,
      nodes: [{ id: 'impl', subject: 'Implement', description: 'Implement from DAG' }],
    }));
    unlinkSync(join(cwd, '.rcs', 'plans', 'test-spec-demo.md'));
  ...
  it('carries approved repository context summary only when the launch gate supplies it', () => {
    ...
    const plan = buildRepoAwareTeamExecutionPlan({
      ...
      approvedRepositoryContextSummary: {
        sourcePath: join(cwd, '.rcs', 'plans', 'repo-context-demo.md'),
