 ...
 * Gated behind the RCS_TEAM_SCALING_ENABLED environment variable.
 *
...
const RCS_TEAM_SCALING_ENABLED_ENV = 'RCS_TEAM_SCALING_ENABLED';
...
export function isScalingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[RCS_TEAM_SCALING_ENABLED_ENV];
  if (!raw) return false;
...
function assertScalingEnabled(env: NodeJS.ProcessEnv = process.env): void {
  if (!isScalingEnabled(env)) {
    throw new Error(
      `Dynamic scaling is disabled. Set ${RCS_TEAM_SCALING_ENABLED_ENV}=1 to enable.`,
    );
  }
}
  ...
  return await withScalingLock(sanitized, leaderCwd, async (): Promise<ScaleUpResult | ScaleError> => {
    ...
    const rollbackScaleUp = async (
      ...
      for (const taskId of createdTaskIds) {
        await rm(join(leaderCwd, '.rcs', 'state', 'team', sanitized, 'tasks', `task-${taskId}.json`), { force: true }).catch(() => {});
      }
    ...
    for (let i = 0; i < count; i++) {
      ...
      const workerDirPath = join(leaderCwd, '.rcs', 'state', 'team', sanitized, 'workers', workerName);
      ...
      if (uniqueTaskRoles.size > 1) {
        console.log(`[rcs:scaling] ${workerName}: mixed task roles [${[...uniqueTaskRoles].join(', ')}], falling back to ${agentType}`);
      }
      ...
      const rolePromptContent = rawRolePromptContent
        ...
        : null;
      const teamInstructionsPath = join(leaderCwd, '.rcs', 'state', 'team', sanitized, 'worker-agents.md');
      ...
      const extraEnv: Record<string, string> = {
        ...
        RCS_TEAM_LEADER_CWD: leaderCwd,
        RCS_MODEL_INSTRUCTIONS_FILE: instructionsFilePath,
      };
      if (workerWorkspace) {
        extraEnv.RCS_TEAM_WORKTREE_PATH = workerWorkspace.worktreePath;
        if (workerWorkspace.branchName) {
          extraEnv.RCS_TEAM_WORKTREE_BRANCH = workerWorkspace.branchName;
        }
        extraEnv.RCS_TEAM_WORKTREE_DETACHED = workerWorkspace.detached ? '1' : '0';
      }
      ...
      const skipReadyWait = env.RCS_TEAM_SKIP_READY_WAIT === '1';
      if (!skipReadyWait) {
        const ready = waitForWorkerReady(sessionName, workerIndex, readyTimeoutMs, paneId);
        if (!ready) {
          console.log(`[rcs:scaling] Warning: worker ${workerName} did not become ready within timeout`);
        }
      }
