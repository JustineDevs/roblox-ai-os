...
#   RCS_TEAM_WORKER_CLI             Worker CLI mode (default: auto)
#   RCS_TEAM_WORKER_CLI_MAP         Comma-separated CLI assignments per worker
#   RCS_TEAM_WORKER_LAUNCH_ARGS     Arguments passed to worker CLIs
#
# Example:
...
if [[ -n "${RCS_TEAM_WORKER_CLI_MAP:-}" ]]; then
  IFS=',' read -ra CLI_MAP_ENTRIES <<< "$RCS_TEAM_WORKER_CLI_MAP"
  if (("${#CLI_MAP_ENTRIES[@]}" != WORKER_COUNT)); then
    echo "error: RCS_TEAM_WORKER_CLI_MAP has ${#CLI_MAP_ENTRIES[@]} entries but WORKER_COUNT is $WORKER_COUNT" >&2
    exit 1
  fi
fi
...
RCS_TEAM_WORKER_CLI_MAP="${RCS_TEAM_WORKER_CLI_MAP:-$(build_default_cli_map "$WORKER_COUNT")}"
RCS_TEAM_WORKER_LAUNCH_ARGS="${RCS_TEAM_WORKER_LAUNCH_ARGS:--c model_reasoning_effort=\"low\"}"
...
echo "RCS_TEAM_WORKER_LAUNCH_ARGS=$RCS_TEAM_WORKER_LAUNCH_ARGS"
...
rcs team api mailbox-mark-notified --input "$MARK_INPUT" --json >/dev/null
rcs team api mailbox-mark-delivered --input "$MARK_INPUT" --json >/dev/null
