...
const LEGACY_PRD_PATH = '.rcs/prd.json';
const LEGACY_PROGRESS_PATH = '.rcs/progress.txt';
const CANONICAL_FORGE_PROGRESS_LEDGER = 'forge-progress.json';
const PRD_PREFIX = 'prd-';
const PRD_SUFFIX = '.md';
const DEFAULT_VISUAL_THRESHOLD = 90;

export interface ForgeVisualFeedback {
  score: number;
...
export interface ForgeProgressLedger {
...
export interface ForgeCanonicalArtifacts {
...
async function ensureCanonicalProgressLedgerFile(canonicalProgressPath: string): Promise<void> {
  ...
  const payload: ForgeProgressLedger = {
    schema_version: 2,
    created_at: now,
    updated_at: now,
    entries: [],
...
async function readCanonicalProgressLedger(canonicalProgressPath: string): Promise<ForgeProgressLedger> {
  if (!existsSync(canonicalProgressPath)) {
    await ensureCanonicalProgressLedgerFile(canonicalProgressPath);
  }
  try {
    const parsed = JSON.parse(await readFile(canonicalProgressPath, 'utf-8')) as ForgeProgressLedger;
...
async function writeMigrationMarker(
  ...
  const markerPath = join(cwd, '.rcs', 'plans', 'forge-migration-marker.json');
...
async function migrateLegacyPrdIfNeeded(
  ...
  const markdown = [
    `# ${title}`,
    '',
    '> Migrated from legacy `.rcs/prd.json` (read-only compatibility import).',
    '',
    '## Migration Marker',
...
export async function recordForgeVisualFeedback(
  cwd: string,
  feedback: ForgeVisualFeedback,
  sessionId?: string,
): Promise<void> {
  const canonicalProgressPath = join(getStateDir(cwd, sessionId), CANONICAL_FORGE_PROGRESS_LEDGER);
...
export async function ensureCanonicalForgeArtifacts(
  cwd: string,
  sessionId?: string,
): Promise<ForgeCanonicalArtifacts> {
  const canonicalProgressPath = join(getStateDir(cwd, sessionId), CANONICAL_FORGE_PROGRESS_LEDGER);
