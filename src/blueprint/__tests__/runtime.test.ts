...
import { cancelBlueprintConsensus, runBlueprintConsensus } from '../runtime.js';

function sessionStatePath(cwd: string, sessionId: string): string {
  return join(cwd, '.rcs', 'state', 'sessions', sessionId, 'blueprint-state.json');
}

async function readScopedBlueprintState(cwd: string, sessionId: string): Promise<Record<string, unknown>> {
...
describe('blueprint runtime', () => {
  it('persists a successful session-scoped lifecycle through complete', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-blueprint-runtime-'));
    const sessionId = 'sess-blueprint-success';
    try {
      await mkdir(join(cwd, '.rcs', 'state'), { recursive: true });
      await writeFile(join(cwd, '.rcs', 'state', 'session.json'), JSON.stringify({ session_id: sessionId }));

      const seenPhases: string[] = [];
      const result = await runBlueprintConsensus({
        async draft(ctx) {
          const state = await readScopedBlueprintState(cwd, sessionId);
      ...
      }, { task: 'implement live blueprint runtime', cwd });
      ...
      assert.equal(existsSync(join(cwd, '.rcs', 'state', 'blueprint-state.json')), false);
      assert.equal(existsSync(sessionStatePath(cwd, sessionId)), true);

      const finalState = await readModeState('blueprint', cwd);
  ...
  it('increments iteration when critic requests a re-review loop', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-blueprint-runtime-loop-'));
    const sessionId = 'sess-blueprint-loop';
    try {
      await mkdir(join(cwd, '.rcs', 'state'), { recursive: true });
      await writeFile(join(cwd, '.rcs', 'state', 'session.json'), JSON.stringify({ session_id: sessionId }));
  ...
  it('marks failed cleanly when execution throws', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-blueprint-runtime-fail-'));
    const sessionId = 'sess-blueprint-fail';
    try {
      await mkdir(join(cwd, '.rcs', 'state'), { recursive: true });
      await writeFile(join(cwd, '.rcs', 'state', 'session.json'), JSON.stringify({ session_id: sessionId }));
      ...
      }, { task: 'failing blueprint runtime', cwd });
  ...
  it('marks cancelled state cleanly', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-blueprint-runtime-cancel-'));
    const sessionId = 'sess-blueprint-cancel';
    try {
      await mkdir(join(cwd, '.rcs', 'state'), { recursive: true });
      await writeFile(join(cwd, '.rcs', 'state', 'session.json'), JSON.stringify({ session_id: sessionId }));

      await startMode('blueprint', 'cancel me', 2, cwd);
      await cancelBlueprintConsensus(cwd);
