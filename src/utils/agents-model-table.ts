...
export const RCS_MODELS_START_MARKER = '<!-- RCS:MODELS:START -->';
export const RCS_MODELS_END_MARKER = '<!-- RCS:MODELS:END -->';
...
export function buildAgentsModelTable(
  ...
  return [
    ...
    '',
    'Auto-generated by `rcs setup` from the current `config.toml` plus RCS model overrides.',
    '',
...
export function upsertAgentsModelTable(
  ...
  const startIndex = content.indexOf(RCS_MODELS_START_MARKER);
  const endIndex = content.indexOf(RCS_MODELS_END_MARKER);

  if (startIndex >= 0 && endIndex > startIndex) {
    const replaceEnd = endIndex + RCS_MODELS_END_MARKER.length;
