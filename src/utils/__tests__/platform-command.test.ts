...
describe('buildPlatformCommandSpec', () => {
  it('wraps .cmd shims through ComSpec on Windows', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-cmd-'));
    try {
  ...
  it('launches .exe binaries directly on Windows', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-exe-'));
    try {
  ...
  it('spawns Git Bash with tmux-stub.sh when tmux resolves to .ps1 beside stub (preserves #{ tokens)', () => {
    const fakeBin = 'C:\\fake\\tmux-stub-bin';
    const ps1Path = join(fakeBin, 'tmux.ps1');
    const stubPath = join(fakeBin, 'tmux-stub.sh');
    const bashPath = join(fakeBin, 'bash.exe');
    const existsImpl = (p: string) => [ps1Path, stubPath, bashPath].includes(p);

    const spec = buildPlatformCommandSpec(
      'tmux',
      ['display-message', '-t', 'session:win.1', '-p', '#{pane_id}'],
      'win32',
      {
        PATH: fakeBin,
        PATHEXT: '.PS1;.EXE',
        RCS_TEST_GIT_BASH: bashPath,
      },
      existsImpl,
    );

    assert.equal(spec.command, bashPath);
    ...
    assert.equal(spec.resolvedPath, bashPath);
  });

  it('resolveWindowsTmuxStubSpawn returns null without a sibling tmux-stub.sh', () => {
    const ps1 = 'C:\\only\\tmux.ps1';
    assert.equal(
      resolveWindowsTmuxStubSpawn(
        ps1,
        ['-V'],
        { RCS_TEST_GIT_BASH: 'C:\\git\\bin\\bash.exe' },
        (p) => p === ps1 || p === 'C:\\git\\bin\\bash.exe',
      ),
      null,
    );
  });
  ...
  it('prefers the npm-installed codex.js entrypoint over Windows shims when available', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-ps1-'));
    try {
  ...
  it('resolves codex.js from npm .bin-style Windows shim layouts', async () => {
    const fakeRoot = await mkdtemp(join(tmpdir(), 'rcs-platform-local-shim-'));
  ...
  it('falls back to cmd shims when the node-hosted codex entrypoint is unavailable', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-cmd-fallback-'));
    try {
      const cmdPath = join(fakeBin, 'codex.cmd');
      await writeFile(cmdPath, '@echo off\r\n');
      const spec = buildPlatformCommandSpec(
  ...
  it('prefers cmd shims over PowerShell shims when no node-hosted entrypoint exists', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-ps1-fallback-'));
    try {
      const ps1Path = join(fakeBin, 'codex.ps1');
      const cmdPath = join(fakeBin, 'codex.cmd');
      await writeFile(ps1Path, '');
  ...
  it('skips directory PATH candidates when resolving the PowerShell executable for .ps1 shims', async () => {
    const shimBin = await mkdtemp(join(tmpdir(), 'rcs-platform-ps1-shim-'));
    const badPowerShellBin = await mkdtemp(join(tmpdir(), 'rcs-platform-bad-powershell-'));
    const goodPowerShellBin = await mkdtemp(join(tmpdir(), 'rcs-platform-good-powershell-'));
    try {
...
describe('resolveCommandPathForPlatform', () => {
  it('prefers PATHEXT candidates on Windows', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-path-'));
    try {
      const exePath = join(fakeBin, 'tmux.exe');
      await writeFile(exePath, '');
      assert.equal(
  ...
  it('falls back to psmux on Windows when tmux is absent', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-psmux-path-'));
    try {
  ...
  it('ignores directory candidates on Windows even when they appear before a real executable', async () => {
    const badPowerShellBin = await mkdtemp(join(tmpdir(), 'rcs-platform-path-dir-'));
    const goodPowerShellBin = await mkdtemp(join(tmpdir(), 'rcs-platform-path-exe-'));
    try {
      const powershellDirectory = join(badPowerShellBin, 'powershell');
      const powershellExePath = join(goodPowerShellBin, 'powershell.exe');
      await mkdir(powershellDirectory, { recursive: true });
  ...
  it('resolves PATH entries to absolute paths on POSIX', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-posix-'));
    try {
...
describe('spawnPlatformCommandSync', () => {
  it('passes the Windows-resolved spec into the spawn implementation', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-spawn-'));
    try {
      const cmdPath = join(fakeBin, 'codex.cmd');
      await writeFile(cmdPath, '@echo off\r\n');
      const calls: Array<{
  ...
  it('launches Windows node-hosted codex through process.execPath without verbatim cmd arguments', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-spawn-node-hosted-'));
    try {
      const cmdPath = join(fakeBin, 'codex.cmd');
      const codexJsPath = join(fakeBin, 'node_modules', '@openai', 'codex', 'bin', 'codex.js');
      await writeFile(cmdPath, '@echo off\r\n');
  ...
  it('does not force verbatim arguments for direct Windows executables', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-spawn-exe-'));
    try {
      const exePath = join(fakeBin, 'tmux.exe');
      await writeFile(exePath, '');
      const calls: Array<{
  ...
  it('launches Windows cmd shims successfully with the real spawn implementation', async () => {
    ...
    const fakeBin = await mkdtemp(join(tmpdir(), 'rcs-platform-spawn-real-'));
    try {
      const cmdPath = join(fakeBin, 'codex.cmd');
  ...
  it('retries blocked node-hosted scripts through process.execPath on non-Windows', () => {
    const scriptPath = '/tmp/rcs-explore-stub.js';
