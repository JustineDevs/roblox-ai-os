...
export const AGENTS_INIT_USAGE = [
  "Usage: rcs agents-init [path] [--dry-run] [--force] [--verbose]",
  "       rcs deepinit [path] [--dry-run] [--force] [--verbose]",
  "",
...
const MANAGED_MARKER = "<!-- RCS:AGENTS-INIT:MANAGED -->";
const MANUAL_START = "<!-- RCS:AGENTS-INIT:MANUAL:START -->";
const MANUAL_END = "<!-- RCS:AGENTS-INIT:MANUAL:END -->";
...
export async function agentsInit(
  ...
  console.log("roblox-ai-os-creator-skills AGENTS bootstrap");
  console.log("===========================\n");
  ...
  for (let index = 0; index < plannedDirs.length; index += 1) {
    ...
    const decision = await syncManagedAgentsFile(
      ...
      backupRoot,
      rootOverlayRisk
        ? "active rcs session detected for project root AGENTS.md"
        : undefined,
    );
  ...
  console.log(
    "- Preserves manual notes only for files already managed by rcs agents-init.",
  );
  console.log(
    "- Never overwrites unmanaged AGENTS.md files unless you pass --force.",
  );
  console.log(
    "- Avoids rewriting project-root AGENTS.md while an active rcs session is running.\n",
  );
