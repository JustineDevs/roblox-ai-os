...
const HELP = [
  'Usage: rcs cleanup [--dry-run]',
  '',
  'Kill orphaned RCS MCP server processes and remove stale RCS /tmp directories left behind by previous Codex App sessions.',
  '',
  'Options:',
...
const RCS_MCP_SERVER_PATTERN = /(?:^|[\\/])dist[\\/]mcp[\\/](?:state|memory|code-intel|trace|wiki)-server\.(?:[cm]?js|ts)\b/i;
const CODEX_PROCESS_PATTERN = /(?:^|[\\/\s])codex(?:\.js)?(?:\s|$)|@openai[\\/]codex/i;
const RCS_LAUNCH_PROCESS_PATTERN = /(?:^|[\\/\s])rcs(?:\.js)?(?:\s|$)|(?:^|[\\/])(?:bin|dist[\\/]cli)[\\/]rcs\.js(?:\s|$)|roblox-ai-os-creator-skills[\\/]dist[\\/]cli[\\/]rcs\.js/i;
const RCS_TMP_DIRECTORY_PATTERN = /^(omc|rcs|roblox-ai-os-creator-skills)-/;
...
export function isRcsMcpProcess(command: string): boolean {
  const normalized = normalizeCommand(command);
  return RCS_MCP_SERVER_PATTERN.test(normalized);
}
...
function listWindowsRcsProcesses(
...
export function listRcsProcesses(
  runCommand: ProcessListCommandRunner = defaultProcessListCommandRunner,
): ProcessEntry[] {
  if (process.platform === 'win32') return listWindowsRcsProcesses(runCommand);
...
function isRcsLaunchProcess(command: string): boolean {
  return RCS_LAUNCH_PROCESS_PATTERN.test(normalizeCommand(command));
}
...
export function findCleanupCandidates(
  ...
  return processes
    ...
    .filter((processEntry) => isRcsMcpProcess(processEntry.command))
...
export function findLaunchSafeCleanupCandidates(
  ...
  return findCleanupCandidates(processes, currentPid).filter((candidate) => {
    ...
    return (
      ...
      !hasAncestorMatching(processByPid, candidate.pid, isRcsLaunchProcess)
    );
  });
}
...
export async function cleanupRcsMcpProcesses(
  args: readonly string[],
  ...
  const listProcessesImpl = dependencies.listProcesses ?? listRcsProcesses;
  ...
  if (candidates.length === 0) {
    writeLine(dryRun
      ? 'Dry run: no orphaned RCS MCP server processes found.'
      : 'No orphaned RCS MCP server processes found.');
    return {
      dryRun,
      candidates,
      terminatedCount: 0,
  ...
  if (dryRun) {
    writeLine(`Dry run: would terminate ${candidates.length} orphaned RCS MCP server process(es):`);
  ...
  writeLine(`Found ${candidates.length} orphaned RCS MCP server process(es). Sending SIGTERM...`);
  ...
  writeLine(`Killed ${terminatedCount} orphaned RCS MCP server process(es)${forceKilledCount > 0 ? ` (${forceKilledCount} required SIGKILL)` : ''}.`);
...
export async function cleanupStaleTmpDirectories(
  ...
  for (const entry of await listTmpEntries(tmpRoot)) {
    if (!entry.isDirectory() || !RCS_TMP_DIRECTORY_PATTERN.test(entry.name)) continue;
  ...
  if (staleDirectories.length === 0) {
    writeLine(dryRun
      ? 'Dry run: no stale RCS /tmp directories found.'
      : 'No stale RCS /tmp directories found.');
    return 0;
  }

  const summaryTarget = formatPlural(
    staleDirectories.length,
    'stale RCS /tmp directory',
    'stale RCS /tmp directories',
  );
  if (dryRun) {
...
export async function cleanupCommand(
  ...
  const cleanupProcesses = dependencies.cleanupProcesses ?? cleanupRcsMcpProcesses;
