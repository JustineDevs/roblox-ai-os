...
const LOCK_ROOT = join(process.cwd(), '.rcs', 'test-locks');
...
export async function withPackagedExploreHarnessHidden<T>(fn: () => Promise<T>): Promise<T> {
  return withPackagedExploreHarnessLock(async () => {
    ...
    const packagedBinary = join(packageBinDir, process.platform === 'win32' ? 'rcs-explore-harness.exe' : 'rcs-explore-harness');
    const packagedMeta = join(packageBinDir, 'rcs-explore-harness.meta.json');
