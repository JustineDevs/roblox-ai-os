...
async function writePrdJson(
  ...
  await writeFile(join(cwd, '.rcs', 'prd.json'), JSON.stringify(payload, null, 2));
}

describe('rcs ralph --prd smoke gate', () => {
  it('aborts before Codex launch when .rcs/prd.json is missing', async () => {
    const cwd = await initRepo('rcs-ralph-prd-smoke-');
    ...
    try {
      ...
      const result = runRcsCli(cwd, ['ralph', '--prd', 'ship release checklist'], buildEnv(home, fakeBin));
      if (shouldSkipForSpawnPermissions(result.error)) return;

      assert.notEqual(result.status, 0, result.error || result.stderr || result.stdout);
      assert.match(`${result.stderr}\n${result.stdout}`, /Missing required PRD\.json at \.rcs\/prd\.json/);
  ...
  it('launches Codex exactly once when .rcs/prd.json is valid', async () => {
    const cwd = await initRepo('rcs-ralph-prd-smoke-');
    const home = join(cwd, 'home');
    const fakeBin = join(cwd, 'bin');
    const launchLog = join(cwd, 'codex-launch.log');
