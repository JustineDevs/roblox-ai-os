...
describe('rcs forge --prd smoke gate', () => {
  it('aborts before Codex launch when .rcs/prd.json is missing', async () => {
    const cwd = await initRepo('rcs-forge-prd-smoke-');
    ...
    try {
      ...
      const result = runRcsCli(cwd, ['forge', '--prd', 'ship release checklist'], buildEnv(home, fakeBin));
