...
const CURRENT_SESSION_PROCESSES: ProcessEntry[] = [
  ...
  { pid: 701, ppid: 700, command: 'node /repo/bin/rcs.js cleanup --dry-run' },
  {
    pid: 710,
    ppid: 700,
  ...
  },
  {
    pid: 900,
    ppid: 1,
    command: 'node /tmp/not-rcs/other-server.js',
  },
];

describe('findCleanupCandidates', () => {
  it('does not treat legacy team-server entrypoints as active RCS MCP processes', () => {
    assert.equal(isRcsMcpProcess('node /tmp/worktree/dist/mcp/team-server.js'), false);
  });

  it('selects orphaned RCS MCP processes while preserving the current session tree', () => {
    assert.deepEqual(
  ...
  it('limits launch-safe cleanup to RCS MCP processes with no live Codex or RCS launch ancestor', () => {
    assert.deepEqual(
  ...
  it('keeps detached MCP candidates whose ancestor chain is live but unrelated to Codex or RCS launchers', () => {
...
describe('listRcsProcesses', () => {
  it('parses valid Windows process discovery rows on win32', () => {
    const originalPlatform = Object.getOwnPropertyDescriptor(process, 'platform');
    Object.defineProperty(process, 'platform', { value: 'win32', configurable: true });
    try {
      const processes = listRcsProcesses(() => [
        JSON.stringify({ pid: 800, ppid: 1, command: 'node C:/tmp/roblox-ai-os-creator-skills/dist/mcp/state-server.js' }),
        JSON.stringify({ pid: 810, ppid: 42, command: 'node C:/tmp/roblox-ai-os-creator-skills/dist/mcp/trace-server.js' }),
      ].join('\n'));
      assert.deepEqual(processes, [
        { pid: 800, ppid: 1, command: 'node C:/tmp/roblox-ai-os-creator-skills/dist/mcp/state-server.js' },
        { pid: 810, ppid: 42, command: 'node C:/tmp/roblox-ai-os-creator-skills/dist/mcp/trace-server.js' },
      ]);
    } finally {
  ...
  it('falls back to BusyBox-compatible args field when command field is unsupported', () => {
    ...
    const processes = listRcsProcesses((file, args) => {
      ...
      return [
        '  800     1 node /tmp/roblox-ai-os-creator-skills/dist/mcp/memory-server.js',
        '  810    42 node /tmp/roblox-ai-os-creator-skills/dist/mcp/trace-server.js --verbose',
      ].join('\n');
    });

    assert.deepEqual(processes, [
      { pid: 800, ppid: 1, command: 'node /tmp/roblox-ai-os-creator-skills/dist/mcp/memory-server.js' },
      { pid: 810, ppid: 42, command: 'node /tmp/roblox-ai-os-creator-skills/dist/mcp/trace-server.js --verbose' },
    ]);
  ...
  it('feeds parsed Windows rows through existing cleanup candidate selection unchanged', () => {
    ...
    Object.defineProperty(process, 'platform', { value: 'win32', configurable: true });
    try {
      const parsed = listRcsProcesses(() => [
        JSON.stringify({ pid: 700, ppid: 500, command: 'codex' }),
        JSON.stringify({ pid: 701, ppid: 700, command: 'node C:/repo/bin/rcs.js cleanup --dry-run' }),
        JSON.stringify({ pid: 710, ppid: 700, command: 'node C:/repo/dist/mcp/state-server.js' }),
        JSON.stringify({ pid: 800, ppid: 1, command: 'node C:/tmp/roblox-ai-os-creator-skills/dist/mcp/memory-server.js' }),
        JSON.stringify({ pid: 810, ppid: 42, command: 'node C:/tmp/worktree/dist/mcp/trace-server.js' }),
        JSON.stringify({ pid: 900, ppid: 1, command: 'node C:/tmp/not-rcs/other-server.js' }),
      ].join('\n'));
      ...
      assert.deepEqual(findCleanupCandidates(parsed, 701), [
        {
          pid: 800,
          ppid: 1,
          command: 'node C:/tmp/roblox-ai-os-creator-skills/dist/mcp/memory-server.js',
          reason: 'ppid=1',
        },
        {
          pid: 810,
...
describe('cleanupRcsMcpProcesses', () => {
  it('supports dry-run without sending signals', async () => {
    const lines: string[] = [];
    let signalCount = 0;

    const result = await cleanupRcsMcpProcesses(['--dry-run'], {
      currentPid: 701,
    ...
    assert.match(lines.join('\n'), /Dry run: would terminate 4 orphaned RCS MCP server process/);
    assert.match(lines.join('\n'), /PID 800/);
    assert.match(lines.join('\n'), /PID 810/);
  });
  ...
  it('sends SIGTERM, waits, and escalates with SIGKILL when needed', async () => {
    ...
    const result = await cleanupRcsMcpProcesses([], {
      currentPid: 701,
      listProcesses: () => [
    ...
    assert.match(lines.join('\n'), /Killed 2 orphaned RCS MCP server process\(es\) \(1 required SIGKILL\)\./);
  });
  ...
  it('supports launch-safe candidate selection for automatic cleanup', async () => {
    ...
    assert.match(lines.join('\n'), /Found 2 orphaned RCS MCP server process/);
...
describe('cleanupStaleTmpDirectories', () => {
  const tmpEntries = [
    { name: 'rcs-stale-a', isDirectory: () => true },
    { name: 'omc-stale-b', isDirectory: () => true },
    { name: 'roblox-ai-os-creator-skills-fresh', isDirectory: () => true },
    { name: 'roblox-ai-os-creator-skills-file', isDirectory: () => false },
  ...
  it('supports dry-run and reports stale matching directories older than one hour', async () => {
    ...
    const removedCount = await cleanupStaleTmpDirectories(['--dry-run'], {
      ...
      statPath: async (path) => ({
        mtimeMs:
          path === '/tmp/roblox-ai-os-creator-skills-fresh'
            ? now - 30 * 60 * 1000
            : now - 2 * 60 * 60 * 1000,
      }),
    ...
    assert.match(
      lines.join('\n'),
      /Dry run: would remove 2 stale RCS \/tmp directories:/,
    );
    assert.match(lines.join('\n'), /\/tmp\/omc-stale-b/);
    assert.match(lines.join('\n'), /\/tmp\/rcs-stale-a/);
    assert.doesNotMatch(lines.join('\n'), /roblox-ai-os-creator-skills-fresh/);
    assert.doesNotMatch(lines.join('\n'), /other-stale/);
  });
  ...
  it('removes only stale matching directories and returns the removed count', async () => {
    ...
    assert.deepEqual(removedPaths, ['/tmp/omc-stale-b', '/tmp/rcs-stale-a']);
    assert.match(lines.join('\n'), /Removed stale \/tmp directory: \/tmp\/omc-stale-b/);
    assert.match(lines.join('\n'), /Removed stale \/tmp directory: \/tmp\/rcs-stale-a/);
    assert.match(lines.join('\n'), /Removed 2 stale RCS \/tmp directories\./);
  });
});
