...
function runRcsCli(
  ...
  const result = spawnSync('node', [rcsBin, ...argv], {
    cwd,
    encoding: 'utf-8',
...
describe('resolveSparkShellBinaryPath', () => {
  it('prefers RCS_SPARKSHELL_BIN override', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-sparkshell-override-'));
    try {
      const binary = join(cwd, 'bin', 'custom-sparkshell');
      assert.equal(
        resolveSparkShellBinaryPath({
          cwd,
          env: { RCS_SPARKSHELL_BIN: './bin/custom-sparkshell' },
          packageRoot: '/unused',
        }),
        binary,
      );
  ...
  it('falls back from packaged binary to repo-local build artifact', () => {
    ...
    const packaged = join(packageRoot, 'bin', 'native', `${process.platform}-${process.arch}`, process.platform === 'win32' ? 'rcs-sparkshell.exe' : 'rcs-sparkshell');
  ...
  it('checks Linux musl packaged paths before glibc and legacy paths', () => {
    assert.deepEqual(
      ...
      [
        '/repo/bin/native/linux-x64-musl/rcs-sparkshell',
        '/repo/bin/native/linux-x64-glibc/rcs-sparkshell',
        '/repo/bin/native/linux-x64/rcs-sparkshell',
      ],
    );
  });
  ...
  it('tries Linux musl packaged binaries before glibc fallbacks', () => {
    const packageRoot = '/repo';
    const seen: string[] = [];
    const glibcPath = '/repo/bin/native/linux-x64-glibc/rcs-sparkshell';
  ...
  it('hydrates a native binary when packaged and repo-local binaries are absent', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-sparkshell-hydrated-'));
    try {
      ...
      const archiveName = process.platform === 'win32'
        ? 'rcs-sparkshell-x86_64-pc-windows-msvc.zip'
      ...
      const buildArchive = process.platform === 'win32'
        ...
        : spawnSync('tar', ['-czf', archivePath, '-C', stagingDir, 'rcs-sparkshell'], { encoding: 'utf-8' });
      ...
      const manifestPath = join(assetRoot, 'native-release-manifest.json');
      await writeFile(manifestPath, JSON.stringify({
        ...
        assets: [{
          ...
          arch: 'x64',
          archive: archiveName,
          binary: process.platform === 'win32' ? 'rcs-sparkshell.exe' : 'rcs-sparkshell',
          binary_path: process.platform === 'win32' ? 'rcs-sparkshell.exe' : 'rcs-sparkshell',
          sha256: checksum,
          size: archiveBuffer.length,
          download_url: archivePath,
        }],
      }, null, 2));
      ...
      const resolved = await resolveSparkShellBinaryPathWithHydration({
        ...
        arch: 'x64',
        env: {
          RCS_NATIVE_MANIFEST_URL: manifestPath,
  ...
  it('falls back cleanly when hydration manifest is unavailable', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-sparkshell-hydration-missing-'));
    try {
...
describe('runSparkShellBinary', () => {
  it('passes argv directly to the native sidecar', () => {
    ...
    runSparkShellBinary('/fake/rcs-sparkshell', ['git', 'diff --stat', 'a|b'], {
      cwd: '/tmp/example',
      env: { TEST_ENV: '1' },
    ...
    assert.deepEqual(invoked, {
      binaryPath: '/fake/rcs-sparkshell',
      args: ['git', 'diff --stat', 'a|b'],
      stdio: ['ignore', 'pipe', 'pipe'],
    });
  });

  it('merges .rcs-config.json env overrides behind explicit shell env', async () => {
    const codexHome = await mkdtemp(join(tmpdir(), 'rcs-sparkshell-config-env-'));
    await writeFile(join(codexHome, '.rcs-config.json'), JSON.stringify({
      env: {
        RCS_DEFAULT_FRONTIER_MODEL: 'frontier-local',
        RCS_DEFAULT_STANDARD_MODEL: 'standard-local',
        RCS_DEFAULT_SPARK_MODEL: 'spark-local',
        RCS_SPARKSHELL_MODEL_INSTRUCTIONS_FILE: '/config/sparkshell-instructions.md',
      },
    }));

    try {
      let invokedEnv: NodeJS.ProcessEnv | undefined;
      runSparkShellBinary('/fake/rcs-sparkshell', ['git', 'status'], {
        cwd: codexHome,
        env: {
          CODEX_HOME: codexHome,
          RCS_DEFAULT_FRONTIER_MODEL: 'frontier-shell',
        },
      ...
      assert.equal(invokedEnv?.RCS_DEFAULT_FRONTIER_MODEL, 'frontier-shell');
      assert.equal(invokedEnv?.RCS_DEFAULT_STANDARD_MODEL, 'standard-local');
      assert.equal(invokedEnv?.RCS_DEFAULT_SPARK_MODEL, 'spark-local');
      assert.equal(invokedEnv?.RCS_SPARKSHELL_MODEL_INSTRUCTIONS_FILE, '/config/sparkshell-instructions.md');
    } finally {
  ...
  it('defaults to packaged lightweight instructions outside the role prompts directory', () => {
    ...
    assert.match(
      invokedEnv?.RCS_SPARKSHELL_MODEL_INSTRUCTIONS_FILE || '',
  ...
  it('detects GLIBC symbol version failures from the native loader', () => {
    assert.equal(
      isSparkShellNativeCompatibilityFailure({
        ...
        output: [],
        stdout: '',
        stderr: "rcs-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.39' not found\n",
        status: 1,
        signal: null,
      }),
      true,
  ...
  it('ignores non-compatibility stderr failures', () => {
    assert.equal(
      isSparkShellNativeCompatibilityFailure({
        ...
        output: [],
        stdout: '',
        stderr: 'rcs sparkshell: summary unavailable (tmux failed)\n',
        status: 1,
        signal: null,
      }),
      false,
...
describe('rcs sparkshell', () => {
  it('includes sparkshell in top-level help output', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-sparkshell-help-'));
    try {
      const result = runRcsCli(cwd, ['--help']);
      if (shouldSkipForSpawnPermissions(result.error)) return;

      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /rcs explore\s+Default read-only exploration entrypoint \(may adaptively use sparkshell backend\)/);
      assert.match(result.stdout, /rcs sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /rcs sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
  ...
  it('prints sparkshell usage when invoked with --help', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-sparkshell-subhelp-'));
    try {
      const result = runRcsCli(cwd, ['sparkshell', '--help']);
      if (shouldSkipForSpawnPermissions(result.error)) return;

      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /Usage: rcs sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /or: rcs sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('preserves child stdout, stderr, and exit code through the JS bridge', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-sparkshell-bridge-'));
    try {
      const binDir = join(cwd, 'bin');
      const stubPath = join(binDir, process.platform === 'win32' ? 'rcs-sparkshell.cmd' : 'rcs-sparkshell');
      ...
      const result = runRcsCli(cwd, ['sparkshell', 'git', 'status'], {
        RCS_SPARKSHELL_BIN: stubPath,
      });
      if (shouldSkipForSpawnPermissions(result.error)) return;
  ...
  it('falls back to raw execution when the packaged native binary is GLIBC-incompatible', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-sparkshell-glibc-fallback-'));
    try {
      const testDir = dirname(fileURLToPath(import.meta.url));
      const repoRoot = join(testDir, '..', '..', '..');
      ...
      const binDir = join(cacheDir, packageJson.version, `${process.platform}-${process.arch}`, 'rcs-sparkshell');
      await mkdir(binDir, { recursive: true });
      const stubPath = join(binDir, process.platform === 'win32' ? 'rcs-sparkshell.exe' : 'rcs-sparkshell');
      await writeFile(
        stubPath,
        process.platform === 'win32'
          ? '@echo off\r\n>&2 echo rcs-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.39\' not found\r\nexit /b 1\r\n'
      ...
      const result = runRcsCli(cwd, ['sparkshell', 'node', '-e', 'process.stdout.write("raw-fallback\\n")'], {
        RCS_NATIVE_CACHE_DIR: cacheDir,
      });
      if (shouldSkipForSpawnPermissions(result.error)) return;
  ...
  it('fails clearly when the configured native binary path does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-sparkshell-missing-'));
    try {
      const missingBinary = join(cwd, 'bin', 'does-not-exist');
      const result = runRcsCli(cwd, ['sparkshell', 'ls'], {
        RCS_SPARKSHELL_BIN: missingBinary,
      });
      if (shouldSkipForSpawnPermissions(result.error)) return;
