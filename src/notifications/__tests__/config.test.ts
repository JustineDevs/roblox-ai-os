...
const ENV_KEYS = [
  'RCS_DISCORD_NOTIFIER_BOT_TOKEN',
  'RCS_DISCORD_NOTIFIER_CHANNEL',
  'RCS_DISCORD_WEBHOOK_URL',
  'RCS_DISCORD_MENTION',
  'RCS_TELEGRAM_BOT_TOKEN',
  'RCS_TELEGRAM_NOTIFIER_BOT_TOKEN',
  'RCS_TELEGRAM_CHAT_ID',
  'RCS_TELEGRAM_NOTIFIER_CHAT_ID',
  'RCS_TELEGRAM_NOTIFIER_UID',
  'RCS_SLACK_WEBHOOK_URL',
  'RCS_SLACK_MENTION',
  'RCS_REPLY_ENABLED',
  'RCS_REPLY_DISCORD_USER_IDS',
  'RCS_REPLY_POLL_INTERVAL_MS',
  'RCS_REPLY_RATE_LIMIT',
];
...
describe('buildConfigFromEnv', () => {
  ...
  it('builds discord-bot config from env vars', () => {
    process.env.RCS_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.RCS_DISCORD_NOTIFIER_CHANNEL = '123456';
  ...
  it('includes validated mention in discord-bot config', () => {
    process.env.RCS_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.RCS_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.RCS_DISCORD_MENTION = '<@12345678901234567>';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('rejects invalid mention in env var', () => {
    process.env.RCS_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.RCS_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.RCS_DISCORD_MENTION = '@everyone';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds discord webhook config from env var', () => {
    process.env.RCS_DISCORD_WEBHOOK_URL = 'https://discord.com/api/webhooks/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds telegram config from env vars', () => {
    process.env.RCS_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.RCS_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds slack config from env var', () => {
    process.env.RCS_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('includes a validated slack mention in slack config', () => {
    process.env.RCS_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
    process.env.RCS_SLACK_MENTION = '<!here>';
  ...
  it('uses RCS_TELEGRAM_NOTIFIER_BOT_TOKEN as fallback', () => {
    process.env.RCS_TELEGRAM_NOTIFIER_BOT_TOKEN = '123:fallback';
    process.env.RCS_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('uses RCS_TELEGRAM_NOTIFIER_UID as fallback for chat ID', () => {
    process.env.RCS_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.RCS_TELEGRAM_NOTIFIER_UID = 'uid-999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds config with multiple platforms from env', () => {
    process.env.RCS_DISCORD_NOTIFIER_BOT_TOKEN = 'bot-token';
    process.env.RCS_DISCORD_NOTIFIER_CHANNEL = 'channel-123';
    process.env.RCS_TELEGRAM_BOT_TOKEN = '456:tg';
    process.env.RCS_TELEGRAM_CHAT_ID = 'chat-789';
    process.env.RCS_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
