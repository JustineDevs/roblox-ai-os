...
describe('getVerbosity', () => {
  const origEnv = process.env.RCS_NOTIFY_VERBOSITY;

  afterEach(() => {
    if (origEnv === undefined) {
      delete process.env.RCS_NOTIFY_VERBOSITY;
    } else {
      process.env.RCS_NOTIFY_VERBOSITY = origEnv;
    }
  });
