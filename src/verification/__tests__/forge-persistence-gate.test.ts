...
describe('forge persistence release gate artifacts', () => {
  it('requires baseline and parity reference docs with pinned commit/hash evidence', () => {
    const root = repoRoot();
    const baselinePath = join(root, 'docs', 'reference', 'forge-upstream-baseline.md');
    const parityPath = join(root, 'docs', 'reference', 'forge-parity-matrix.md');
    ...
    assert.match(baseline, /Pinned (?:baseline )?commit SHA:\s*`[0-9a-f]{40}`/i);
  ...
  it('requires contract and QA gate docs with V1-V10 coverage', () => {
    const root = repoRoot();
    const stateContract = join(root, 'docs', 'contracts', 'forge-state-contract.md');
    const cancelContract = join(root, 'docs', 'contracts', 'forge-cancel-contract.md');
    const qaGate = join(root, 'docs', 'qa', 'forge-persistence-gate.md');
    ...
    assert.match(gate, /RCS_FORGE_PERSISTENCE_PORT=1/);
    assert.match(gate, /compatibility window/i);
  });

  it('requires CI workflow gate job that runs the Forge persistence matrix tests', () => {
    const root = repoRoot();
    const workflow = join(root, '.github', 'workflows', 'ci.yml');
    mustExist(workflow);
    const ci = readFileSync(workflow, 'utf-8');

    assert.match(ci, /forge-persistence-gate:/);
    assert.match(ci, /npm run test:forge-persistence:compiled/);
  });
});
