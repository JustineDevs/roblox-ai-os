...
describe('sidecar tmux launcher', () => {
  it('builds a detached horizontal right-side split running watch mode', () => {
    const args = buildSidecarTmuxSplitArgs({ cwd: '/repo', teamName: 'demo-team', width: 52, sessionId: 'sess 1', rcsBin: '/repo/dist/cli/rcs.js' });
    ...
    assert.match(args[10], /RCS_SESSION_ID='sess 1'/);
    assert.match(args[10], /node '\/repo\/dist\/cli\/rcs\.js' sidecar 'demo-team' --watch --width 52/);
  });

  it('uses a safe minimum sidecar width and parses the new pane id', () => {
    const command = buildSidecarWatchCommand({ cwd: '/repo', teamName: 'demo', width: 10, rcsBin: '/repo/rcs.js' });
    assert.match(command, /--width 48$/);

    const paneId = launchSidecarTmuxPane(
      { cwd: '/repo', teamName: 'demo', width: 48, rcsBin: '/repo/rcs.js' },
      (args) => {
