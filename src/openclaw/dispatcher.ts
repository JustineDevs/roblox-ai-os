 ...
 *
 * SECURITY: Command gateway requires RCS_OPENCLAW_COMMAND=1 opt-in.
 ...
 * gateway timeout > RCS_OPENCLAW_COMMAND_TIMEOUT_MS > default.
 */
export function resolveCommandTimeoutMs(
  gatewayTimeout?: number,
  envTimeoutRaw: string | undefined = process.env.RCS_OPENCLAW_COMMAND_TIMEOUT_MS,
): number {
 ...
 * - Requires RCS_OPENCLAW_COMMAND=1 opt-in (separate gate from RCS_OPENCLAW)
 * - Timeout is configurable via gateway.timeout or RCS_OPENCLAW_COMMAND_TIMEOUT_MS
...
export async function wakeCommandGateway(
  ...
  if (process.env.RCS_OPENCLAW_COMMAND !== "1") {
    return {
      gateway: gatewayName,
      success: false,
      error: "Command gateway disabled (set RCS_OPENCLAW_COMMAND=1 to enable)",
    };
  }
