 ...
 * Wiki Storage
 *
 * File I/O layer for the RCS wiki knowledge base.
 */
...
import { rcsWikiDir } from '../utils/paths.js';
import {
  type WikiLogEntry,
  type WikiPage,
...
export function getWikiDir(root: string): string {
  return rcsWikiDir(root);
}

export function ensureWikiDir(root: string): string {
  const wikiDir = getWikiDir(root);
  mkdirSync(wikiDir, { recursive: true });
  const rcsRoot = join(root, '.rcs');
  mkdirSync(rcsRoot, { recursive: true });
  const gitignorePath = join(rcsRoot, '.gitignore');
