...
describe('session lifecycle manager', () => {
  it('resets session metrics files with zeroed counters', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-metrics-'));
    try {
      await resetSessionMetrics(cwd);

      const metricsPath = join(cwd, '.rcs', 'metrics.json');
      const hudPath = join(cwd, '.rcs', 'state', 'hud-state.json');
  ...
  it('writes hud session metrics into the active session scope when session id is provided', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-metrics-scoped-'));
    try {
      await resetSessionMetrics(cwd, 'sess-scoped');

      const metricsPath = join(cwd, '.rcs', 'metrics.json');
      const hudPath = join(cwd, '.rcs', 'state', 'sessions', 'sess-scoped', 'hud-state.json');
      assert.equal(existsSync(metricsPath), true);
      assert.equal(existsSync(hudPath), true);
  ...
  it('treats symlinked cwd aliases as authoritative for the same session state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-cwd-alias-'));
  ...
  it('writes session start/end lifecycle artifacts and archives session history', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-lifecycle-'));
    ...
    try {
      ...
      const historyPath = join(cwd, '.rcs', 'logs', 'session-history.jsonl');
      ...
      const dailyLogPath = join(cwd, '.rcs', 'logs', `rcs-${todayIsoDate()}.jsonl`);
  ...
  it('removes canonical and native session-scoped hud state on session end', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-end-hud-cleanup-'));
  ...
  it('preserves canonical session id while reconciling native SessionStart metadata', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-native-reconcile-'));
    try {
      await writeSessionStart(cwd, 'rcs-launch-1');
      ...
      assert.equal(reconciled.session_id, 'rcs-launch-1');
      ...
      assert.equal(persisted?.session_id, 'rcs-launch-1');
  ...
  it('starts a fresh canonical session when a new native SessionStart arrives after an earlier native session', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-native-fresh-'));
    try {
      await writeSessionStart(cwd, 'rcs-old-session', {
  ...
  it('falls back to a fresh canonical session when reconciling without authoritative launch state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-native-fallback-'));
    try {
      const statePath = join(cwd, '.rcs', 'state', 'session.json');
      await resetSessionMetrics(cwd);
  ...
  it('treats invalid session JSON as absent state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-invalid-'));
    try {
      const statePath = join(cwd, '.rcs', 'state', 'session.json');
      await resetSessionMetrics(cwd);
  ...
  it('ignores session.json when its recorded cwd points at another worktree', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-mismatched-cwd-'));
    try {
      const statePath = join(cwd, '.rcs', 'state', 'session.json');
      await resetSessionMetrics(cwd);
      await writeFile(statePath, JSON.stringify({
  ...
  it('ignores session.json when its PID identity is stale', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-session-stale-pointer-'));
    try {
      const statePath = join(cwd, '.rcs', 'state', 'session.json');
      await resetSessionMetrics(cwd);
      await writeFile(statePath, JSON.stringify({
        ...
        pid: 4242,
        pid_start_ticks: 11,
        pid_cmdline: 'node rcs',
      }), 'utf-8');

      const state = await readUsableSessionState(cwd, {
        platform: 'linux',
        isPidAlive: () => true,
        readLinuxIdentity: () => ({ startTicks: 22, cmdline: 'node rcs' }),
      });
      assert.equal(state, null);
    } finally {
      await rm(cwd, { recursive: true, force: true });
...
describe('isSessionStale', () => {
  it('returns false for a live Linux process when identity matches', () => {
    ...
    const stale = isSessionStale(state, {
      platform: 'linux',
      isPidAlive: () => true,
      readLinuxIdentity: () => ({ startTicks: 111, cmdline: 'node rcs' }),
    });
  ...
  it('returns true for PID reuse on Linux when start ticks mismatch', () => {
    ...
    const stale = isSessionStale(state, {
      platform: 'linux',
      isPidAlive: () => true,
      readLinuxIdentity: () => ({ startTicks: 222, cmdline: 'node rcs' }),
    });
