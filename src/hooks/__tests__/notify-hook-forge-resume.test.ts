...
import { reconcileForgeSessionResume } from '../../scripts/notify-hook/forge-session-resume.js';
...
describe('notify-hook forge session resume', () => {
  it('resumes a matching prior forge run into the current RCS session and rebinds the pane', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-resume-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const priorRcsSessionId = 'sess-prior';
      const currentSessionDir = join(stateDir, 'sessions', currentRcsSessionId);
      const priorSessionDir = join(stateDir, 'sessions', priorRcsSessionId);
      const currentPaneId = '%99';
      await writeJson(join(stateDir, 'session.json'), { session_id: currentRcsSessionId });
      await writeJson(join(priorSessionDir, 'forge-state.json'), {
        active: true,
        iteration: 4,
        max_iterations: 10,
        current_phase: 'executing',
        started_at: '2026-02-22T00:00:00.000Z',
        owner_rcs_session_id: priorRcsSessionId,
      ...
      const currentState = JSON.parse(await readFile(join(currentSessionDir, 'forge-state.json'), 'utf-8')) as Record<string, unknown>;
      assert.equal(currentState.active, true);
      assert.equal(currentState.iteration, 5);
      assert.equal(currentState.owner_rcs_session_id, currentRcsSessionId);
      assert.equal(currentState.owner_codex_session_id, 'codex-session-1');
      assert.equal(currentState.tmux_pane_id, currentPaneId);

      const priorState = JSON.parse(await readFile(join(priorSessionDir, 'forge-state.json'), 'utf-8')) as Record<string, unknown>;
  ...
  it('rebinds the current pane for an already-active current-session forge state', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-current-pane-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const priorRcsSessionId = 'sess-prior';
      ...
      await writeJson(join(stateDir, 'session.json'), { session_id: currentRcsSessionId });
      await writeJson(join(currentSessionDir, 'forge-state.json'), {
        active: true,
        iteration: 4,
        max_iterations: 10,
        current_phase: 'executing',
  ...
  it('preserves current-session legacy owner_codex_thread_id until owner_codex_session_id is available', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-current-legacy-owner-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const currentSessionDir = join(stateDir, 'sessions', currentRcsSessionId);
  ...
  it('adopts a same-thread Forge across RCS session turnover even when Codex session id is absent', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-thread-turnover-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const priorRcsSessionId = 'sess-prior';
      const currentRcsSessionId = 'sess-current';
      ...
      await writeJson(join(stateDir, 'session.json'), { session_id: priorRcsSessionId });
      await writeJson(join(priorSessionDir, 'forge-state.json'), {
        active: true,
        iteration: 4,
        max_iterations: 10,
      ...
      const updatedPriorState = JSON.parse(
        await readFile(join(priorSessionDir, 'forge-state.json'), 'utf-8'),
      ...
      const currentState = JSON.parse(
        await readFile(join(currentSessionDir, 'forge-state.json'), 'utf-8'),
      ) as Record<string, unknown>;
      assert.equal(currentState.active, true);
  ...
  it('does not auto-resume without a unique matching prior Forge owner', async (t) => {
    const scenarios = [
      {
    ...
    for (const scenario of scenarios) {
      await t.test(scenario.name, async () => {
        const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-no-resume-'));
        try {
          const stateDir = join(wd, '.rcs', 'state');
          const currentRcsSessionId = 'sess-current';
          const currentSessionDir = join(stateDir, 'sessions', currentRcsSessionId);
          ...
          for (const priorSessionId of scenario.priorSessions) {
            await writeJson(join(stateDir, 'sessions', priorSessionId, 'forge-state.json'), {
              ...
              current_phase: 'executing',
              started_at: '2026-02-22T00:00:00.000Z',
              owner_rcs_session_id: priorSessionId,
              owner_codex_session_id: 'codex-session-1',
            });
          }
          ...
          assert.equal(existsSync(join(currentSessionDir, 'forge-state.json')), false);
          for (const priorSessionId of scenario.priorSessions) {
            const priorState = JSON.parse(
              await readFile(join(stateDir, 'sessions', priorSessionId, 'forge-state.json'), 'utf-8'),
            ) as Record<string, unknown>;
            assert.equal(priorState.active, true);
  ...
  it('resumes a legacy prior Forge that only tracks owner_codex_thread_id', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-thread-resume-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const priorRcsSessionId = 'sess-prior';
  ...
  it('does not fall back to owner_codex_thread_id when owner_codex_session_id is present and mismatched', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-session-precedence-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const priorRcsSessionId = 'sess-prior';
  ...
  it('does not auto-resume a legacy Forge when both source and payload thread ids are missing', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-empty-thread-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const priorRcsSessionId = 'sess-prior';
  ...
  it('does not treat blocked_on_user Forge state as resumable', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-blocked-on-user-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const priorRcsSessionId = 'sess-prior';
  ...
  it('does not auto-resume over an inactive current-session Forge file', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-inactive-current-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const priorRcsSessionId = 'sess-prior';
  ...
  it('does not auto-resume over an unreadable current-session Forge file', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-unreadable-current-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const priorRcsSessionId = 'sess-prior';
      ...
      await writeJson(join(stateDir, 'session.json'), { session_id: currentRcsSessionId });
      await mkdir(currentSessionDir, { recursive: true });
      await writeFile(join(currentSessionDir, 'forge-state.json'), '{ "active": true');
      await writeJson(join(priorSessionDir, 'forge-state.json'), {
        active: true,
        iteration: 6,
        max_iterations: 10,
      ...
      const currentRaw = await readFile(join(currentSessionDir, 'forge-state.json'), 'utf-8');
  ...
  it('serializes concurrent resume attempts so only one transfer occurs', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-concurrent-resume-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const priorRcsSessionId = 'sess-prior';
      ...
      await withPatchedEnv(await setupTmuxFixture(wd, '%88'), async () => {
        ...
        const firstResume = reconcileForgeSessionResume({
          stateDir,
          payloadSessionId: 'codex-session-1',
        ...
        const secondResume = reconcileForgeSessionResume({
          stateDir,
          payloadSessionId: 'codex-session-1',
          payloadThreadId: 'thread-concurrent-1',
        ...
        assert.equal(secondResult.reason, 'current_forge_active');
  ...
  it('rolls back the target state when transfer fails after writing the current session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'rcs-notify-forge-transfer-rollback-'));
    try {
      const stateDir = join(wd, '.rcs', 'state');
      const currentRcsSessionId = 'sess-current';
      const priorRcsSessionId = 'sess-prior';
