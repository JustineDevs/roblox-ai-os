...
describe("deep-interview Ouroboros contract", () => {
	...
	it("documents surface-aware rcs question handling and fallback boundaries", () => {
		assert.match(deepInterviewSkill, /rcs question/i);
		assert.match(
			deepInterviewSkill,
		...
		assert.match(
			deepInterviewSkill,
			/attached-tmux Codex CLI, deep-interview uses `rcs question`/i,
		);
		assert.match(
			deepInterviewSkill,
			/RCS_QUESTION_RETURN_PANE=\$TMUX_PANE/i,
		);
		assert.match(
			deepInterviewSkill,
			/outside tmux and cannot render `rcs question`, use (the )?native structured (question tool|input) when available/i,
		);
		assert.match(
			deepInterviewSkill,
	...
	it("teaches canonical single-choice vs multi-answerable rcs question payloads", () => {
		assert.match(
			deepInterviewSkill,
	...
	it("uses RCS-native output paths", () => {
		assert.match(deepInterviewSkill, /\.rcs\/interviews\//);
		assert.match(deepInterviewSkill, /\.rcs\/specs\//);
	});
	...
	it("documents the autoresearch specialization contract", () => {
		...
		assert.match(
			deepInterviewSkill,
			/\.rcs\/specs\/deep-interview-autoresearch-\{slug\}\.md/i,
		);
...
describe("cross-skill and AGENTS coherence for deep-interview", () => {
	...
	it("makes template AGENTS explicit about surface-aware deep-interview questioning", () => {
		assert.match(templateAgents, /deep-interview is active in attached-tmux RCS CLI\/runtime.*`rcs question`/i);
		assert.match(templateAgents, /after launching `rcs question` in a background terminal, wait for that terminal to finish and read the JSON answer before continuing/i);
		assert.match(templateAgents, /RCS_QUESTION_RETURN_PANE=\$TMUX_PANE/i);
		assert.match(templateAgents, /Outside tmux or native surfaces that cannot render `rcs question` should use the native structured question path when available/i);
