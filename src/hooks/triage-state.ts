 ...
 *   Without session id: .rcs/state/prompt-routing-state.json
 *
 * Rules:
...
function resolveStatePath(workingDirectory?: string, sessionId?: string | null): string | null {
  ...
  // getStateDir returns .rcs/state/sessions/<sessionId>/ when sessionId is provided,
  // or .rcs/state/ as the root fallback — exactly the paths we need.
