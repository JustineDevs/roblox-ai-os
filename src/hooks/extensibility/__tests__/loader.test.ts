...
describe('hooksDir', () => {
  it('returns .rcs/hooks under cwd', () => {
    assert.equal(hooksDir('/project'), join('/project', '.rcs', 'hooks'));
  });
});
...
describe('ensureHooksDir', () => {
  it('creates .rcs/hooks directory and returns path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-ensure-'));
    try {
      const dir = await ensureHooksDir(cwd);
      assert.equal(dir, join(cwd, '.rcs', 'hooks'));
...
describe('discoverHookPlugins', () => {
  it('returns empty array when hooks directory does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-discover-'));
    try {
...
describe('validateHookPluginExport', () => {
  it('returns valid for plugin with onHookEvent export', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-validate-'));
    try {
...
describe('loadHookPluginDescriptors', () => {
  it('returns empty array when no hooks directory', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'rcs-load-'));
    try {
