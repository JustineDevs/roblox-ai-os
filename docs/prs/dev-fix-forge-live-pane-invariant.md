...
# PR Draft: Preserve verified Forge pane anchors and avoid tmux-hook target drift

## Target branch
`dev`

## Summary
This follow-up fixes pane rebinding drift in the managed tmux recovery paths that showed up during review of the Forge resume / continue work.

The main issue was stored-pane drift. Once a Forge pane anchor had already been verified as part of the correct managed session, recovery could still rescan the whole tmux session and rebind to whichever Codex pane was currently focused, or fall back to a shell-degraded pane that only historically started as Codex. In multi-pane Codex sessions that could silently move future watcher or auto-nudge injections onto the wrong lane.
...
- teach the Forge continue watcher to re-resolve stored pane anchors against the current managed session before sending continue nudges
...
## Why this is good
- prevents Forge continue / resume state from jumping between Codex panes just because focus changed
...
## Validation
- [x] `npx biome lint src/scripts/notify-hook/managed-tmux.ts src/scripts/notify-fallback-watcher.ts src/scripts/notify-hook/auto-nudge.ts src/hooks/__tests__/notify-hook-managed-tmux.test.ts src/hooks/__tests__/notify-hook-tmux-heal.test.ts src/hooks/__tests__/notify-fallback-watcher.test.ts src/hooks/__tests__/notify-hook-auto-nudge.test.ts docs/prs/dev-fix-forge-live-pane-invariant.md`
- [x] `npm run build`
...
## Related
- follow-up to the Forge resume / managed tmux healing review pass
