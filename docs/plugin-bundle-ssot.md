...
- **Plugin MCP metadata:** `src/config/rcs-first-party-mcp.ts` is canonical. `plugins/roblox-ai-os-creator-skills/.mcp.json` must match `buildRcsPluginMcpManifest()`.
...
5. `rcs setup` writes generated TOML to `.codex/agents/<name>.toml` for the selected scope and installs setup-owned prompts to `.codex/prompts/<name>.md`.
