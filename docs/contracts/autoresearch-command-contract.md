# `rcs autoresearch` parity contract

`rcs autoresearch` is a thin supervisor that drives one Codex experiment session per iteration while RCS owns the durable keep/discard/reset loop.

## CLI

```bash
rcs autoresearch <mission-dir> [codex-args...]
rcs autoresearch --resume <run-id> [codex-args...]
rcs autoresearch --help
```

- Fresh launch always creates a new run-tagged lane.
- `--resume <run-id>` loads `.rcs/logs/autoresearch/<run-id>/manifest.json`.
- A second launch is rejected while repo-root `.rcs/state/autoresearch-state.json` points at an active run.
...
- worktree `<repo>.rcs-worktrees/autoresearch-<mission-slug>-<run-tag>`
- repo-root run artifacts under `.rcs/logs/autoresearch/<run-id>/`

Repo-root state responsibilities:
- `.rcs/state/autoresearch-state.json` = active-run pointer/lock only
- `.rcs/logs/autoresearch/<run-id>/manifest.json` = authoritative per-run state
- `.rcs/logs/autoresearch/<run-id>/candidate.json` = candidate handoff from the just-finished Codex session
- `.rcs/logs/autoresearch/<run-id>/iteration-ledger.json` = durable iteration history
- `.rcs/logs/autoresearch/<run-id>/latest-evaluator-result.json` = latest evaluator output
