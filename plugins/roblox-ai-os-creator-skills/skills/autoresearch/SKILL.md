...
## Do not use when
- You want the old `rcs autoresearch` command surface (hard-deprecated)
...
2. **Persist mode state** in `.rcs/state/.../autoresearch-state.json` including:
   - `validation_mode`
...
2. Materialize `.rcs/specs/autoresearch-{slug}/mission.md`, `sandbox.md`, and `result.json`.
