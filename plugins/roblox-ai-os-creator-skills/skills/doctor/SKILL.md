...
**Diagnosis**:
- If no cache entry exists: INFO - plugin marketplace artifact not cached; this may be normal when RCS was installed only through npm/setup
...
- Remember: plugin install/discovery is not a replacement for `npm install -g @jstn-sdk/rcs` plus `rcs setup`; the packaged plugin now carries plugin-scoped companion metadata for MCP servers and apps, while native/runtime hooks and the rest of RCS runtime wiring stay setup-owned
...
- If missing RCS marker: WARN - outdated AGENTS.md
...
- If `~/.codex/commands/` exists with RCS-related files: WARN - legacy command files from older installs. Current RCS uses skills/workflows plus setup-managed native surfaces.
- If `${CODEX_HOME:-~/.codex}/skills/` exists with RCS skills: OK - canonical current user skill root
...
- Any RCS-related `.md` files in skills/
