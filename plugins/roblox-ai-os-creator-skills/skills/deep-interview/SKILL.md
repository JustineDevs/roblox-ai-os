...
<Purpose>
Deep Interview is an intent-first Socratic clarification loop before planning or implementation. It turns vague ideas into execution-ready specifications by asking targeted questions about why the user wants a change, how far it should go, what should stay out of scope, and what RCS may decide without confirmation.
</Purpose>
...
- **Autoresearch (`--autoresearch`)**: same interview rigor as Standard, but specialized for `$autoresearch` mission readiness and `.rcs/specs/` artifact handoff

If no flag is provided, use **Standard**.

<Mode_Flags>
- **`--autoresearch`**: switch the interview into autoresearch-intake mode for `$autoresearch` handoff. In this mode, the interview should converge on a validator-ready research mission, write canonical artifacts under `.rcs/specs/`, and preserve the explicit `refine further` vs `launch` boundary for downstream skill intake.
</Mode_Flags>
</Depth_Profiles>
...
- When session guidance enables `USE_RCS_EXPLORE_CMD`, prefer `rcs explore` for simple read-only brownfield fact gathering; keep prompts narrow and concrete, and keep ambiguous or non-shell-only investigation on the richer normal path and fall back normally if `rcs explore` is unavailable.
...
- In attached-tmux Codex CLI, deep-interview uses `rcs question` as the required RCS-owned structured questioning path for every interview round
- When invoking `rcs question` through attached-tmux Bash/tool paths, preserve the leader-pane return target by prefixing the command with `RCS_QUESTION_RETURN_PANE=$TMUX_PANE` (or a concrete `%pane` value)
- If you launch `rcs question` in a background terminal, immediately wait for that background terminal to finish and read its JSON answer before scoring ambiguity, asking another round, or handing off
- Treat `answers[]` as the primary `rcs question` success contract. For a single interview round, read `answers[0].answer`; use legacy top-level `answer` only as a compatibility fallback when needed.
- If the current runtime is outside tmux and cannot render `rcs question`, use the native structured question tool when available; otherwise ask exactly one concise plain-text question and wait for the answer
...
2. Attempt to load the latest relevant context snapshot from `.rcs/context/{slug}-*.md`.
...
5. Save snapshot to `.rcs/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) and reference it in mode state.
...
### 2b) Ask the question
Use the surface-appropriate structured questioning path for every interview round. In attached-tmux sessions, use RCS-owned structured questioning via `rcs question` (this is the required structured-question equivalent and required `AskUserQuestion` equivalent for deep-interview). Outside tmux, use native structured input when available; otherwise ask exactly one concise plain-text question and wait for the answer. Present:
...
`rcs question` payload guidance for interview rounds:
- Deep-interview is Socratic: ask one focused round at a time. Do not use batch `questions[]` to combine multiple interview rounds, even though `rcs question` supports batch forms for other workflows.
...
- Decision Boundaries (what RCS may decide without confirmation)
- Constraints
...
- **Canonical artifact path:** `.rcs/specs/deep-interview-autoresearch-{slug}.md`
- **Launch artifact bundle:** `.rcs/specs/autoresearch-{slug}/mission.md`, `.rcs/specs/autoresearch-{slug}/sandbox.md`, and `.rcs/specs/autoresearch-{slug}/result.json`
- **Launch artifact directory:** `.rcs/specs/autoresearch-{slug}/`
- **Required artifact sections:**
  ...
  - `Seed Inputs`
  - `Confirmation Bridge`
- **Required launch artifacts under `.rcs/specs/autoresearch-{slug}/`:**
  - `mission.md`
  - `sandbox.md`
  - `result.json`
...
- Use `rcs question` as the RCS-native structured user-input tool for each interview round when an attached tmux renderer is available
- From attached-tmux Bash/tool paths, call it as `RCS_QUESTION_RETURN_PANE=$TMUX_PANE rcs question ...` unless an explicit `%pane` return target is already known
- If the current runtime is outside tmux and cannot render `rcs question`, use native structured input when available; otherwise ask exactly one concise plain-text question and wait for the answer
- After `rcs question` returns JSON, prefer `answers[0].answer` / `answers[]`; use legacy `answer` only as a fallback for older records
- Use `state_write` / `state_read` for resumable mode state
- If the interview cannot ask a required `rcs question` round, persist the blocker as terminal state with `active: false` and `current_phase: "blocked"`; do not write a terminal blocked phase with `active: true`
- Read/write context snapshots under `.rcs/context/`
- Record whether the oversized-context summary gate is not needed, pending, or satisfied before any scoring or handoff step
- Save transcript/spec artifacts under `.rcs/interviews/` and `.rcs/specs/`
</Tool_Usage>
...
<Final_Checklist>
- [ ] Preflight context snapshot exists under `.rcs/context/{slug}-{timestamp}.md`
