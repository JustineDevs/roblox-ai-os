...
0. **Pre-context intake (required before planning/execution loop starts)**:
   - Assemble or load a context snapshot at `.rcs/context/{task-slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`).
   ...
   - If request ambiguity is high, gather brownfield facts first. When session guidance enables `USE_RCS_EXPLORE_CMD`, prefer `rcs explore` for simple read-only repository lookups with narrow, concrete prompts; otherwise use the richer normal explore path. Then run `$deep-interview --quick <task>` to close critical gaps.
...
5. **Visual task gate (when screenshot/reference images are present)**:
   ...
   - Persist verdict to `.rcs/state/{scope}/ralph-progress.json` including numeric + qualitative feedback.
...
7.5 **Mandatory Deslop Pass**:
   - After Step 7 passes, run `roblox-ai-os-creator-skills:ai-slop-cleaner` on **all files changed during the Ralph session**.
...
- If an `rcs_state` MCP tool call reports that its stdio transport is unavailable/closed, do **not** retry the same MCP call. Retry once through the supported CLI parity surface with the same payload, preserving `workingDirectory` and `session_id`: `rcs state write --input '<json>' --json`, `rcs state read --input '<json>' --json`, or `rcs state clear --input '<json>' --json`. If the CLI path also fails, continue with `.rcs/context` / `.rcs/plans` file-backed artifacts and report the state persistence blocker.
</Tool_Usage>

## State Management

Use the `rcs_state` MCP server tools (`state_write`, `state_read`, `state_clear`) for Ralph lifecycle state.
...
Prompt-side `$ralph` workflow activation is lighter-weight than `rcs ralph --prd ...`.
It seeds Ralph workflow state and guidance, but it does not implicitly launch the
CLI entrypoint or apply the PRD startup gate. Treat `rcs ralph --prd ...` as the
...
1. Run deep-interview in quick mode before creating PRD artifacts:
   ...
   - Persist interview output to `.rcs/interviews/{slug}-{timestamp}.md`
2. Create canonical PRD/progress artifacts:
   - PRD: `.rcs/plans/prd-{slug}.md`
   - Progress ledger: `.rcs/state/{scope}/ralph-progress.json` (session scope when available, else root scope)
...
5. Initialize canonical progress ledger at `.rcs/state/{scope}/ralph-progress.json`
...
Workflow: Detect flag, extract task, create `.rcs/plans/prd-{slug}.md`, create `.rcs/state/{scope}/ralph-progress.json`, begin ralph loop.

### Legacy compatibility
- During the compatibility window, Ralph `--prd` startup still validates machine-readable story state from `.rcs/prd.json`.
- `.rcs/plans/prd-{slug}.md` remains the canonical storage/documentation artifact, but it is not yet the startup validation source.
- If `.rcs/prd.json` exists and canonical PRD is absent, migrate one-way into `.rcs/plans/prd-{slug}.md`.
- If `.rcs/progress.txt` exists and canonical progress ledger is absent, import one-way into `.rcs/state/{scope}/ralph-progress.json`.
