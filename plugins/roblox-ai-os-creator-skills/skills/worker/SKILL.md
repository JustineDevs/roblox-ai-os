...
You MUST be running with `RCS_TEAM_WORKER` set. It looks like:
...
Use CLI interop:
- `rcs team api send-message --input <json> --json` with `{team_name, from_worker, to_worker:"leader-fixed", body}`
...
6. Claim the task (do NOT start work without a claim) using claim-safe lifecycle CLI interop (`rcs team api claim-task --json`).
7. Do the work.
...
Use CLI interop:
- `rcs team api mailbox-list --json` to read
- `rcs team api mailbox-mark-delivered --json` to acknowledge delivery
