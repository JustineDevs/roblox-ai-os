...
1. Invoke RCS runtime directly with `rcs team ...`
...
If `rcs team` is unavailable, stop with a hard error.
...
3. `rcs` command resolves to the intended install/build
4. If running repo-local `node bin/rcs.js ...`, run `npm run build` after `src` changes
...
`rcs team` currently performs:

1. Parse args (`N`, `agent-type`, task)
2. Sanitize team name from task text
3. Initialize team state:
   - `.rcs/state/team/<team>/config.json`
   - `.rcs/state/team/<team>/manifest.v2.json`
   - `.rcs/state/team/<team>/tasks/task-<id>.json`
4. Compose team-scoped worker instructions file at:
   - `.rcs/state/team/<team>/worker-agents.md`
   - Uses project `AGENTS.md` content (if present) + worker overlay, without mutating project `AGENTS.md`
5. Resolve canonical shared state root from leader cwd (`<leader-cwd>/.rcs/state`)
6. Split current tmux window into worker panes
7. Launch workers with:
   - `RCS_TEAM_WORKER=<team>/worker-<n>`
   - `RCS_TEAM_STATE_ROOT=<leader-cwd>/.rcs/state`
   - `RCS_TEAM_LEADER_CWD=<leader-cwd>`
   - worker CLI selected by `RCS_TEAM_WORKER_CLI` / `RCS_TEAM_WORKER_CLI_MAP` (`codex` or `claude`)
...
- Workers may run in separate git worktrees (`rcs team --worktree[=<name>]`) while sharing one team state root
...
- Submit routing uses this CLI resolution order per worker trigger:
  ...
  2) `RCS_TEAM_WORKER_CLI_MAP` entry for that worker index,
  3) fallback `RCS_TEAM_WORKER_CLI` / auto detection.
...
1. Explicit worker model in `RCS_TEAM_WORKER_LAUNCH_ARGS`
2. Inherited leader `--model` flag
3. Low-complexity default from `RCS_DEFAULT_SPARK_MODEL` (legacy alias: `RCS_SPARK_MODEL`) when 1+2 are absent and team `agentType` is low-complexity

Default-model rule:
- Do **not** assume a frontier or spark model from recency or model-family heuristics.
- Use `RCS_DEFAULT_FRONTIER_MODEL` for frontier-default guidance.
- Use `RCS_DEFAULT_SPARK_MODEL` for spark/low-complexity worker-default guidance.
...
- When the leader assigns teammate roles/tasks, RCS allocates **per-worker reasoning effort dynamically** from the resolved worker role (`low`, `medium`, `high`).
- Explicit launch args still win: if `RCS_TEAM_WORKER_LAUNCH_ARGS` already includes `-c model_reasoning_effort=...`, that explicit value overrides dynamic allocation for every worker.
...
2. Monitor task and worker progress with runtime/state tools first (`rcs team status <team>`, `rcs team resume <team>`, mailbox/state files)
3. Wait for terminal task state before shutdown:
   - `pending=0`
   - `in_progress=0`
   - `failed=0` (or explicitly acknowledged failure path)
4. Only then run `rcs team shutdown <team>`
...
- `shutdown`: graceful shutdown request, then cleanup (deletes `.rcs/state/team/<team>`)

## Data Plane and Control Plane

### Control Plane

- tmux panes/processes (`RCS_TEAM_WORKER` per worker)
- leader notifications via `tmux display-message`

### Data Plane

- `.rcs/state/team/<team>/...` files
- Team mailbox files:
- `.rcs/state/team/<team>/mailbox/leader-fixed.json`
- `.rcs/state/team/<team>/mailbox/worker-<n>.json`
- `.rcs/state/team/<team>/dispatch/requests.json` (durable dispatch queue; hook-preferred, fallback-aware)
...
- `.rcs/state/team/<team>/manifest.v2.json`
- `.rcs/state/team/<team>/tasks/task-<id>.json`
- `.rcs/state/team/<team>/workers/worker-<n>/identity.json`
- `.rcs/state/team/<team>/workers/worker-<n>/inbox.md`
- `.rcs/state/team/<team>/workers/worker-<n>/heartbeat.json`
- `.rcs/state/team/<team>/workers/worker-<n>/status.json`
- `.rcs/state/team-leader-nudge.json`


## Team Mutation Interop (CLI-first)

Use `rcs team api` for machine-readable mutation/reads instead of legacy `team_*` MCP tools.
...
- Send ACK to `leader-fixed` mailbox via `rcs team api send-message --json`
- Claim/transition/release task lifecycle via `rcs team api <operation> --json`
...
Use only after checking `rcs team status <team>` and mailbox/state evidence:

1. Capture pane tail to confirm current worker state:
   - `tmux capture-pane -t %<worker-pane> -p -S -120`
   - If a larger-tail read or bounded summary would help, prefer explicit opt-in inspection via `rcs sparkshell --tmux-pane %<worker-pane> --tail-lines 400` before improvising extra tmux commands.
...
2. `.rcs/state/team/<team>/mailbox/leader-fixed.json` exists
3. Worker skill loaded and `rcs team api send-message --json` called
4. Task-id mismatch not blocking worker flow

### Worker logs `rcs team api ... ENOENT` (or legacy `team_send_message ENOENT` / `team_update_task ENOENT`)

Meaning:
- Team state path no longer exists while worker is still running.
- Typical cause: leader/manual flow ran `rcs team shutdown <team>` (or removed `.rcs/state/team/<team>`) before worker finished.

Checks:

1. `rcs team status <team>` and confirm whether tasks were still `in_progress` when shutdown occurred
2. Verify whether `.rcs/state/team/<team>/` exists
3. Inspect worker pane tail for post-shutdown writes
4. Confirm no external cleanup (`rm -rf .rcs/state/team/<team>`) happened during execution
...
- Do not kill HUD pane (`rcs hud --watch`) unless intentionally restarting HUD
...
- `team_cleanup` (**state-server**): Deletes team state **files** on disk (`.rcs/state/team/<team>/`). Use after a team run is fully complete.
- tmux/session cleanup: Use the documented `rcs team` shutdown / cleanup flow when you need to stop worker panes or clean up an interrupted run.

### Automation example

```
1. rcs team 1:executor "fix bugs"
