...
- When session guidance enables `USE_RCS_EXPLORE_CMD`, prefer `rcs explore` for simple read-only repository lookups during planning; keep prompts narrow and concrete, and keep prompt-heavy or ambiguous planning work on the richer normal path and fall back normally if `rcs explore` is unavailable.
...
2. **Ask one focused question** using the surface-appropriate structured question path for preferences, scope, and constraints: in attached-tmux RCS runtime use `rcs question`; outside tmux use native structured input when available; use plain text only as a last fallback
...
1. Read plan file from `.rcs/plans/`
...
- Use the surface-appropriate structured question path for preference questions (scope, priority, timeline, risk tolerance): attached-tmux RCS runtime uses `rcs question`; outside tmux uses native structured input when available. Use plain text only as a last fallback for unsupported surfaces or highly specific free-form values.
- `rcs question` success JSON uses `answers[]` as the primary contract. For single-question planning prompts, read `answers[0].answer`; treat top-level `answer` as legacy compatibility fallback only.
...
| User Preference | "Priority?", "Timeline?" | Ask user via the structured question path (`rcs question` in attached tmux; native structured input where available) |
