...
use rcs_mux::build_capture_pane_args;
...
fn run(args: Vec<String>) -> Result<(), SparkshellError> {
    ...
    match summarize_output(&execution_argv, &output) {
        ...
        Err(error) => {
            write_raw_output(&output.stdout, &output.stderr)?;
            eprintln!("rcs sparkshell: summary unavailable ({error})");
        }
    }
...
fn usage_text() -> String {
    format!(
        concat!(
            ...
            "   or: rcs-sparkshell --tmux-pane <pane-id> [--tail-lines <{min}-{max}>]\n",
            "\n",
