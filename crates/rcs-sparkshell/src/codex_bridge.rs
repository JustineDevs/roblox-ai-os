...
pub fn resolve_model() -> String {
    env::var("RCS_SPARKSHELL_MODEL")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| {
            env::var("RCS_DEFAULT_SPARK_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
        .or_else(|| {
            env::var("RCS_SPARK_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
...
pub fn resolve_fallback_model() -> String {
    env::var("RCS_SPARKSHELL_FALLBACK_MODEL")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| {
            env::var("RCS_DEFAULT_STANDARD_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
...
pub fn resolve_instructions_file() -> Option<String> {
    env::var("RCS_SPARKSHELL_MODEL_INSTRUCTIONS_FILE")
        .ok()
...
pub fn read_summary_timeout_ms() -> u64 {
    env::var("RCS_SPARKSHELL_SUMMARY_TIMEOUT_MS")
        .ok()
...
mod tests {
    ...
    fn model_resolution_prefers_sparkshell_override() {
        let _guard = env_lock();
        unsafe {
            env::set_var("RCS_SPARKSHELL_MODEL", "spark-a");
            env::set_var("RCS_DEFAULT_SPARK_MODEL", "spark-b");
        }
        assert_eq!(resolve_model(), "spark-a");
        unsafe {
            env::remove_var("RCS_SPARKSHELL_MODEL");
            env::remove_var("RCS_DEFAULT_SPARK_MODEL");
            env::remove_var("RCS_SPARK_MODEL");
        }
    }
    ...
    fn fallback_model_resolution_prefers_override_then_default_standard() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("RCS_SPARKSHELL_FALLBACK_MODEL");
            env::remove_var("RCS_DEFAULT_STANDARD_MODEL");
        }
        assert_eq!(resolve_fallback_model(), DEFAULT_STANDARD_MODEL);

        unsafe {
            env::set_var("RCS_DEFAULT_STANDARD_MODEL", "standard-a");
        }
        assert_eq!(resolve_fallback_model(), "standard-a");

        unsafe {
            env::set_var("RCS_SPARKSHELL_FALLBACK_MODEL", "standard-b");
        }
    ...
    fn timeout_defaults_when_unset() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("RCS_SPARKSHELL_SUMMARY_TIMEOUT_MS");
        }
    ...
    fn model_resolution_ignores_blank_override_and_uses_secondary_env() {
        let _guard = env_lock();
        unsafe {
            env::set_var("RCS_SPARKSHELL_MODEL", "   ");
            env::set_var("RCS_DEFAULT_SPARK_MODEL", "spark-b");
        }
    ...
    fn instructions_file_resolution_prefers_override_and_ignores_blank() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("RCS_SPARKSHELL_MODEL_INSTRUCTIONS_FILE");
        }
        assert_eq!(resolve_instructions_file(), None);

        unsafe {
            env::set_var(
                "RCS_SPARKSHELL_MODEL_INSTRUCTIONS_FILE",
        ...
        unsafe {
            env::set_var("RCS_SPARKSHELL_MODEL_INSTRUCTIONS_FILE", "   ");
        }
        assert_eq!(resolve_instructions_file(), None);
    ...
    fn timeout_defaults_for_zero_and_invalid_values() {
        let _guard = env_lock();
        unsafe {
            env::set_var("RCS_SPARKSHELL_SUMMARY_TIMEOUT_MS", "0");
        }
        assert_eq!(read_summary_timeout_ms(), DEFAULT_SUMMARY_TIMEOUT_MS);

        unsafe {
            env::set_var("RCS_SPARKSHELL_SUMMARY_TIMEOUT_MS", "bogus");
        }
        assert_eq!(read_summary_timeout_ms(), DEFAULT_SUMMARY_TIMEOUT_MS);
