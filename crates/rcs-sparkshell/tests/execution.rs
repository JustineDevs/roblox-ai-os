...
fn sparkshell_bin() -> &'static str {
    env!("CARGO_BIN_EXE_rcs-sparkshell")
}
...
fn summary_mode_uses_codex_exec_and_model_override() {
    ...
    let output = Command::new(sparkshell_bin())
        .env("PATH", path)
        .env("RCS_SPARKSHELL_LINES", "1")
        .env("RCS_SPARKSHELL_MODEL", "spark-test-model")
        .arg("sh")
        .arg("-c")
        .arg("printf 'one\ntwo\n'")
        .output()
...
",
    ...
    let output = Command::new(sparkshell_bin())
        ...
        .env("RCS_SPARKSHELL_LINES", "1")
        .env("RCS_SPARKSHELL_MODEL", "spark-test-model")
        .env("RCS_SPARKSHELL_FALLBACK_MODEL", "frontier-test-model")
        .arg("sh")
        .arg("-c")
        .arg("printf 'one\ntwo\n'")
        .output()
